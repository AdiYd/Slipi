  ...
  return (
    <Layout className="min-h-screen z-10">
      ...
      <Sider
        width={200}
        theme={theme}
        className={`card* bg-white/90 dark:bg-neutral-900/90 rounded-lg my-4 shadow-none border-none fixed h-[stretch] backdrop-blur-lg
          ${isMobile ? 'z-50' : 'z-40'} 
        ...
        >
          <div className="flex-1">
                  ...
                  icon={
                    !collapsed ? (
                      <Icon
                        ...
                        className="text-text-light dark:text-text-dark transition-transform duration-300 text-2xl"
                      />
                    ) : (
                      <Icon
                        icon="ic:outline-double-arrow"
                        className="text-text-light rotate-180 transition-transform duration-300 dark:text-text-dark text-2xl"
                      />
                    )
                  }
                  onClick={() => setCollapsed(!collapsed)}
            ...
            <div className="flex flex-col items-center justify-center ">
              <Title level={5} className="w-full text-center">
                תפריט
              </Title>
              {/* {!isMobile && !collapsed &&
      ...
      >
        {!id && !isMobile && (
          <motion.header
            ...
            className={`p-0 flex w-[stretch] mt-4 ml-2 backdrop-blur-xl fixed z-40 min-h-[60px] flex-row-reverse justify-between items-center align-middle bg-white/85 dark:bg-neutral-900/90 px-6 max-sm:px-2 rounded-lg`}
          >
            ...
            <Avatar
              ...
              size={27}
              className="bg-primary-light cursor-pointer ml-3 min-w-7 min-h-7 dark:bg-primary-dark"
              icon={<UserOutlined />}
            />
            {isMobile && (
              <Button
        ...
        <motion.div
          ...
          className={`my-4 relative ${
            id && !isMobile ? '' : 'mt-[84px]'
          } overflow-y-auto p-6 max-sm:p-2 min-h-[280px] bg-white/40 backdrop-blur-xl shadow-none border-none rounded-lg dark:bg-gray-950/40`}
        >
          {children}
        </motion.div>
      </Layout>
