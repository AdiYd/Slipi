...
import { Avatar, Button, Card, Carousel, Col, List, Progress, Row, Spin, Typography } from 'antd';
...
const Dashboard: React.FC = () => {
  ...
  const remainingTrainingsList = trainings?.filter(
    (training) => !user?.trainings.find((ut) => ut.id === training.id && ut.completed)
  );

  const completionPercentage = Math.round(
    (completedTrainingsList.length / trainings?.length) * 100
  );
  const totalDuration = completedTrainingsList.reduce(
  ...
  const chatHistory = user?.trainings
    ...
    .map((chat, id) => ({
      ...
      timestamp: new Date(chat.timestamp).toLocaleTimeString('he-IL', {
        hour: '2-digit',
        minute: '2-digit',
      }),
  ...
  return (
    <DashboardLayout>
      <div className=" text-center items-center mt-4 mb-8">
        ...
        <span className="text-textSecondary-light text-base dark:text-textSecondary-dark">
          פה תוכלו למצוא את כל ההדרכות שלכם,לשאול שאלות ולקבל תמיכה בכל שלב.
        </span>
      </div>
              ...
              <div className="relative items-center my-auto flex gap-8 mt-6 h-full">
                ...
                <div className="text-right mt-auto">
                  <h3 className="text-3xl font-bold mb-2 bg-gradient-to-br from-violet-500 via-fuchsia-500 to-pink-500 bg-clip-text text-transparent">
                    {trainings?.length || 0}
                    <span className="text-base font-normal mr-2 text-textSecondary-light dark:text-textSecondary-dark">
                      הכשרות שונות במערכת
                    </span>
                  </h3>
                  <Title
                    level={5}
                    className="text-textSecondary-light dark:text-textSecondary-dark text-base"
                  >
                    סה״כ הדרכות
                  </Title>
                </div>
        ...
        <Col xs={24} sm={24} md={8} lg={8} xl={8}>
          <motion.div whileHover={{ scale: 1.02 }} transition={{ duration: 0.3 }}>
            <Card className="card* relative group overflow-hidden h-[180px]">
              ...
              {/* Decorative shapes - square pattern */}
              <div className="absolute inset-0 overflow-hidden">
      ...
      <div className="relative overflow-visible my-8">
        ...
        >
          ...
          <Carousel ref={carouselRef} {...settings} className="training-carousel">
            ...
            {trainings?.map((product) => (
              <Col key={product.id} xs={24} sm={12} md={8}>
                <Card
                  hoverable
                  cover={
                    <img alt={product.title} src={product.imageUrl} className="h-48 object-cover" />
                  }
                >
          ...
          <List
            ...
            renderItem={(chat) => (
              <List.Item>
                <List.Item.Meta
                  avatar={
                    <MessageOutlined className="text-primary-light dark:text-primary-dark text-xl" />
                  }
                  title={chat.message}
      ...
      <div className="flex h-24 my-8 gap-4 justify-center items-center"></div>
    </DashboardLayout>
  );
};
